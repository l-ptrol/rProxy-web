...
6. **Версионирование**: При каждом коммите, вносящем изменения в код скрипта `rproxy` или `install.sh`, необходимо повышать значение переменной `VERSION` (минимум патч-версию, например 1.2.0 -> 1.2.1). Версии в `rproxy.py`, `main.py` и `install.sh` должны быть синхронизированы.

> [!IMPORTANT]
> Нарушение этих правил ведет к снижению качества взаимодействия. При любых сомнениях — используй русский язык.
7. **Синхронизация версий**: При изменении номера версии в любом из файлов (`rproxy.py`, `main.py`, `install.sh`), необходимо синхронно обновить его во всех остальных указанных файлах.
